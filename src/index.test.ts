...
describe("identity", () => {
  ...
  it("composes standalone — it declares no dependsOn", () => {
    // The template must compose alone — it is what a new extension starts from.
    expect(manifest.dependsOn ?? []).toHaveLength(0);
  });
});

describe("permissions", () => {
  it("declares the one placeholder permission every slot references", () => {
    expect(manifest.permissions?.map((p) => p.id)).toEqual(["template:read"]);
  });
  ...
  it("gates the page and the widget on read, never on nothing", () => {
    ...
    for (const route of manifest.routes ?? []) {
      expect(route.permission, `route ${route.pattern} is ungated`).toBe("template:read");
    }
    for (const widget of manifest.widgets ?? []) {
      expect(widget.permission, `widget ${widget.id} is ungated`).toBe("template:read");
    }
  });
...
describe("nav, routes and widgets", () => {
  ...
  it("points every specifier at its OWN package", () => {
    ...
    for (const spec of specifiers) {
      expect(spec, `${spec} does not live in this package`).toMatch(
        /^@tracht-digital-solutions\/tds-ext-template\//,
      );
    }
  });
  ...
  it("points the widget at the endpoint its island actually reads", () => {
    expect(manifest.widgets?.[0]?.dataEndpoint).toBe("/template/summary");
  });

  it("namespaces its nav, widget and settings ids under the module", () => {
    // These share one namespace across every composed extension.
    for (const n of manifest.nav ?? []) expect(n.id).toMatch(/^template/);
    for (const w of manifest.widgets ?? []) expect(w.id).toMatch(/^template/);
    for (const s of manifest.settings ?? []) expect(s.id).toMatch(/^template/);
  });
...
describe("i18n", () => {
  ...
  it("namespaces its i18n keys so they cannot collide with another extension", () => {
    for (const key of Object.keys(manifest.i18n?.de ?? {})) {
      expect(key, `${key} is not namespaced`).toMatch(/^template\./);
    }
  });
});
...
describe("composition", () => {
  ...
  it("surfaces its route, nav entry and settings panel through the composition", () => {
    ...
    expect(composed.routes.map((r) => r.pattern)).toContain("/template");
    expect(composed.nav.map((n) => n.id)).toContain("template");
    expect(composed.settings.map((s) => s.id)).toContain("template");
  });
});
